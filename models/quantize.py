...
def conv2d_biprec(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1, num_bits_grad=None):
    # out1 is with respect to weight and bias, 
    # it is used to calculate the gradient of *weight* and *bias*.
    out1 = F.conv2d(input.detach(), weight, bias,
                    stride, padding, dilation, groups) 

    # out2 is with respect to input,
    # it is used to calculate the gradient of *input*.
...
class RangeBN(nn.Module):
    ...
    def forward(self, x):
        x = self.quantize_input(x) # x is the input
        
        # If x is of 2 dimensions, 
        # then x should be the output of linear operator.
        if x.dim() == 2:  # 1d
        ...
        if self.training:
            ...
            mean_max = y.max(-1)[0].mean(-1)  # mean_max is for every channel
            mean_min = y.min(-1)[0].mean(-1)  # mean_min is for every channel
            mean = y.view(C, -1).mean(-1)  # also, mean is also for every channel
