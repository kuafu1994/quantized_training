...
from quantize import quantize, QConv2d, QLinear, RangeBN

__all__ = ['resnet50_quantized']
...
def init_model(model):
    for m in model.modules():
        if isinstance(m, QConv2d):
            n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels # C_o * H_f * W_f
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        # for QConv, QLinear and RangeBN, they will quantize the input anyway
        out = self.conv1(x) 
        out = self.bn1(out)
        out = self.relu(out)
...
class ResNet_imagenet(ResNet):
    ...
    def __init__(self, num_classes=1000,
        ...
        self.avgpool = nn.AvgPool2d(7) # Why kernel_size is 7, since the feature of the last layer is 7 x  7
...
def resnet50_quantized(num_classes=1000):
