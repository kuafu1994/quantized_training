...
def main():
    ...
    # save_path is ./results/*time_step*/
    ...
    # Here we should define the resume is false.
    # Since the checkpoint file will store in another path.
    results = ResultsLog(
        results_path, title='Training Results - %s' % args.save, resume=False)
    ...
    # It is not necessary to define the model config in the args
    ...
    # We'd better not use the evaluate option
    if args.evaluate:
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None):
    ...
    # In this example, there is only one GPU.
    ...
    # target should be the label 
    for i, (inputs, target) in enumerate(data_loader): # It seems that dataloader is a iter
...
def train(data_loader, model, criterion, epoch, optimizer):
    ...
    # This has any effect only on certain modules, Dropout, BatchNorm will be effected by the mode.
    model.train()
